...
class PMMInventoryShift(ScriptStrategyBase):
    ...
    Description:
    The bot extends the PMM Price Shift script with an additional price shift based onbid inventory position.
    """
    bid_spread = 0.0002
    ask_spread = 0.0002
    ...
    bid_spread_scalar = 60
    ask_spread_scalar = 30
    ...
    # Initializes candles
    candles_config = CandlesConfig(connector=candle_exchange,
    ...
    def create_proposal(self) -> List[OrderCandidate]:
        ...
        buy_order = OrderCandidate(trading_pair=self.trading_pair, is_maker=True, order_type=OrderType.LIMIT_MAKER,
                                   order_side=TradeType.BUY, amount=Decimal(self.order_amount), price=buy_price)

        sell_order = OrderCandidate(trading_pair=self.trading_pair, is_maker=True, order_type=OrderType.LIMIT_MAKER,
