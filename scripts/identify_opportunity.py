...
#     """
    ...
    # api_key = < api_key >
    # api_secret = < api_secret >class IdentifyOpportunity(ScriptStrategyBase):
class IdentifyOpportunity(ScriptStrategyBase):
    def __init__(self, connectors: Dict[str, ConnectorBase]):
        super().__init__(connectors)
        self.connectors = connectors
        self._tradable_assets = None

    @property
    def tradable_assets(self):
        if self._tradable_assets is None:
            self._tradable_assets = self.collect_tradable_assets()
        return self._tradable_assets

    def collect_tradable_assets(self):
        columns = ['symbol', 'baseAsset', 'quoteAsset', 'permissions', 'ocoAllowed',
                   'isMarginTradingAllowed', 'isSpotTradingAllowed']
        tradable_assets = []

        for i in self.connectors.get_exchange_info()['symbols']:
            if i['status'] == 'TRADING':
                tradable_assets.append(list(map(i.get, columns)))

        tradable_assets = pd.DataFrame(tradable_assets, columns=columns)
        return tradable_assets[tradable_assets['quoteAsset'] == 'USD']
    # def __init__(self, api_key: str, api_secret: str, exchange: str = "binance_us_paper_trade", tld: str = 'us'):
    #     self.client = self.initialize_client(api_key, api_secret, tld)
    #     self.tradable_assets = self.collect_tradable_assets()


    # # Below section of the code gives us all the assets that are tradable
    # columns = ['symbol', 'baseAsset', 'quoteAsset', 'permissions', 'ocoAllowed', 'isMarginTradingAllowed',
