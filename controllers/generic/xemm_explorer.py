...
class XEMMExplorerControllerConfig(ControllerConfigBase):
    ...
    min_order_amount_in_quote: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt=lambda
                e: "Enter the minimum order amount in quote. Typically the minimum allowed by the exchange. Example: 10 for 10 USDT for BNB-USDT: ",
            prompt_on_new=True
        ))
    paper_trade_amount_in_quote: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt=lambda
                e: "Enter the simulated order amount for calculating the profitability. Example: 1000 for 1000 USDT for BNB-USDT: ",
            prompt_on_new=True
        ))
...
class XEMMExplorerController(ControllerBase):
    ...
    def determine_executor_actions(self) -> List[ExecutorAction]:
        ...
        for trading_pair in self.config.trading_pairs:
            ...
            if len(active_executors_on_pair) == 0:
                ...
                mid_price = self.market_data_provider.get_price_by_type(self.config.maker_exchange, trading_pair,
                                                                        PriceType.MidPrice)
                base_asset, quote_asset = split_hb_trading_pair(trading_pair)
                base_balance = self.market_data_provider.connectors[self.config.maker_exchange].get_available_balance(
                    base_asset)
                total_quote_balance = self.market_data_provider.connectors[self.config.maker_exchange].get_balance(
                    quote_asset)
                ...
                if should_create_executor:
                    ...
                    self.logger().info(
                        f"Not enough {quote_asset} balance to open new {maker_side.name} executor for {trading_pair}")
    ...
    def to_format_status(self) -> List[str]:
        """
        Generate a formatted status report of the controller's performance.
