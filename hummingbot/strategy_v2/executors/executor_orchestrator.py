...
from hummingbot.strategy_v2.executors.assignment_adapter_executor.assignment_adapter_executor import (
    AssignmentAdapterExecutor,
)
...
class ExecutorOrchestrator:
    ...
    def execute_action(self, action: ExecutorAction):
        ...
        self.logger().info(f"Executing action for controller: {controller_id}, action type: {type(action).__name__}")
    ...
    def create_executor(self, action: CreateExecutorAction):
        ...
        #
        # # Inspect config initialization
        # self.logger().info("Inspecting how this config was created:")
        # Get the module and class name properly
        type_module = type(executor_config).__module__
        type_name = type(executor_config).__name__
        # self.logger().info(f"Module: {type_module}, Class: {type_name}")
        
        # Check for type confusion by checking different classes
        # self.logger().info("Checking for type confusion:")
        # Direct instance check
        is_assignment = isinstance(executor_config, AssignmentExecutorConfig)
        is_position = isinstance(executor_config, PositionExecutorConfig)
        is_assignment_adapter = isinstance(executor_config, AssignmentAdapterExecutorConfig)
        # self.logger().info(f"Direct isinstance checks: is_assignment={is_assignment}, is_position={is_position}, is_assignment_adapter={is_assignment_adapter}")
        
        # Type string check
        is_type_assignment = "assignment" in type_name.lower() or "assignment" in type_module.lower()
        is_type_position = "position" in type_name.lower() or "position" in type_module.lower()
        self.logger().info(f"Type string checks: is_type_assignment={is_type_assignment}, is_type_position={is_type_position}")
        
        # Field check - does it have fields unique to one type or the other?
        has_assignment_id = hasattr(executor_config, 'assignment_id')
        # self.logger().info(f"Field check: has_assignment_id={has_assignment_id}")
        
        # Type property check
        type_property = getattr(executor_config, 'type', None)
        # self.logger().info(f"Type property value: {type_property}")
        
        # Try to inspect the module directly
        # try:
        #     # Find if this is the actual position executor config class
        #     from hummingbot.strategy_v2.executors.position_executor.data_types import PositionExecutorConfig as Original_PEC
        #     from hummingbot.strategy_v2.executors.assignment_executor.data_types import AssignmentExecutorConfig as Original_AEC
        #     from hummingbot.strategy_v2.executors.assignment_adapter_executor.data_types import AssignmentAdapterExecutorConfig as Original_AAEC
        #
        #     # self.logger().info(f"Identity check with original class:")
        #     # self.logger().info(f"type(config) is Original_PEC: {type(executor_config) is Original_PEC}")
        #     # self.logger().info(f"type(config) is Original_AEC: {type(executor_config) is Original_AEC}")
        #     # self.logger().info(f"type(config) is Original_AAEC: {type(executor_config) is Original_AAEC}")
        #     #
        #     # # Check if these classes are properly distinct
        #     # self.logger().info(f"Original_PEC is Original_AEC: {Original_PEC is Original_AEC}")
        #     # self.logger().info(f"Original_PEC is Original_AAEC: {Original_PEC is Original_AAEC}")
        #     # self.logger().info(f"Original_AEC is Original_AAEC: {Original_AEC is Original_AAEC}")
        # except Exception as e:
        #     self.logger().error(f"Error during module inspection: {str(e)}")

        # For now, we replace the controller ID in the executor config with the actual controller object to maintain
        # compatibility
        ...
        if type_property == "position_executor":
         ...
         # FALLBACK: If no type field or unknown value, use the class type
            ...
            # self.logger().info("Creating PositionExecutor based on class type (fallback)")
            executor = PositionExecutor(self.strategy, executor_config, self.executors_update_interval)
