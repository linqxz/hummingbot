...
class XEMMExplorerExecutor(ExecutorBase):
    ...
    def liquidate_base_assets(self):
        ...
        """
        self.logger().info(
            f"Liquidating remaining base assets of {self.config.trading_pair} on {self.config.maker_exchange}")
    ...
    def validate_sufficient_balance(self):
        ...
        if maker_adjusted_candidate.amount == Decimal("0"):
            ...
            self.logger().error(
                f"Not enough budget to open a position on {self.config.trading_pair}. Shutting down executor.")
            self.stop()
    ...
    async def create_maker_order(self):
        ...
        self.logger().info(
            f"Sent maker order id = {order_id} at price {self.maker_target_price} on {self.config.trading_pair}")
    ...
    async def control_update_maker_order(self):
        ...
        if trade_profitability < self.config.min_profitability:
            self.logger().info(
                f"Trade profitability {trade_profitability} on {self.config.trading_pair} is below minimum profitability. Cancelling order.")
        ...
        if trade_profitability > self.config.max_profitability:
            self.logger().info(
                f"Trade profitability {trade_profitability} on {self.config.trading_pair} is above target profitability. Cancelling order.")
            self._strategy.cancel(self.config.maker_exchange, self.config.trading_pair, self.maker_order.order_id)
    ...
    def process_order_failed_event(self, _, market, event: MarketOrderFailureEvent):
        ...
        if self.maker_order and self.maker_order.order_id == event.order_id:
            ...
            self.maker_order = None
            self.logger().info(
                f"Failed to open maker order id = {event.order_id} on {self.config.trading_pair}, retry = {self._current_retries}")
    ...
    def evaluate_max_retries(self):
        ...
        if self._current_retries > self._max_retries:
            ...
            self.logger().error(
                f"Reached max retries {self._max_retries} to open maker order on {self.config.trading_pair}. Shutting down executor.")
            self.stop()
    ...
    def get_net_pnl_pct(self) -> Decimal:
        ...
        if self._is_trade_completed():
            if self.config.maker_side == TradeType.SELL:
                pnl = (
                                  self.maker_order.average_executed_price - self.taker_hedge_price) / self.maker_order.average_executed_price
            else:
                pnl = (
                                  self.taker_hedge_price - self.maker_order.average_executed_price) / self.maker_order.average_executed_price
